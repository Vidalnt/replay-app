...
import type { Config } from "../config/config.mts";
...
class _ApplicationStarter {
  ...
  async start(outputEntryFile: string): Promise<void> {
    ...
    this._electronProcess = spawn(path.resolve(`node_modules/.bin/${_electronBin}`), [outputEntryFile, ...this._args], {
      stdio: "inherit",
    });
...
export class Dev extends Cli {
  ...
  async init(): Promise<void> {
    ...
    const outputEntryFile = path.join(
      this._config.main.fileConfig.output.dir,
      this._config.main.fileConfig.output.filename,
    );
    await rimraf(outputEntryFile);
    const start = () => this._applicationStarter.start(outputEntryFile);

    _logger.debug("Starting dev builders");
    await this._mainBuilder.build();
    await this._mainBuilder.dev(start);
    this._rendererBuilder?.dev(() => {});
